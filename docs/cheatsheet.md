...
agentkg onboard    # uses getpass.getuser() → correct
agentkg profile    # same default → correct
...
agentkg init --person egs

# 2. Build your profile via the structured interview
agentkg onboard --person egs

# 3. Verify it took
agentkg profile --person egs
...
```bash
agentkg ingest "We decided to use OAuth2 with PKCE." --role user
agentkg ingest "Understood, I'll implement it that way." --role assistant

# Defer embedding (fast — run consolidate later)
agentkg ingest "quick note" --role user --no-embed
```
...
```bash
agentkg query "authentication strategy" --k 10

# Also search profile nodes (preferences, commitments, style)
agentkg query "docstring style" --k 8 --include-profile
```
...
agentkg assemble "what did we decide about auth?" --budget 4000
```
...
agentkg snapshot
agentkg snapshot --label "before refactor"
```
...
agentkg onboard --person egs --update

# Skip the optional Personal phase
agentkg onboard --person egs --skip-optional
```
...
```json
{
  "hooks": {
    "UserPromptSubmit": [{
      "hooks": [{
        "type": "command",
        "command": "PROMPT=$(jq -r '.prompt'); REPO_ROOT=\"$(git rev-parse --show-toplevel)\"; agentkg-ingest \"$PROMPT\" --role user --repo \"$REPO_ROOT\" --no-embed 2>/dev/null || true",
        "async": true
      }]
    }],
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "REPO_ROOT=\"$(git rev-parse --show-toplevel)\"; agentkg-snapshot --repo \"$REPO_ROOT\" --label \"session-end\" 2>/dev/null || true",
        "async": true
      }]
    }]
  }
